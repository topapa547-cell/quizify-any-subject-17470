...
import { class9EnglishNCERTData } from './class9EnglishNCERT';

export const insertClass9EnglishNCERT = async () => {
  try {
    const processedData = class9EnglishNCERTData.map(solution => ({
      class_level: 9,
      subject: 'english',
    ...
    console.log(`Attempting to insert ${processedData.length} Class 9 English NCERT solutions...`);
    ...
    console.error('Unexpected error during Class 9 English NCERT insertion:', error);
    return {
      success: false,
      total: class9EnglishNCERTData.length,
      successCount: 0,
